...
census_2010 <- census_2010 %>% 
  dplyr::mutate(pct_white = (P008003/P001001)*100,
  ...
  dplyr::select(GEOID, NAME, total_pop, pct_white, pct_black, pct_hisp, pct_asian, pct_young_male)
...
acs_2017 <- acs_2017 %>%
  dplyr::select (GEOID, NAME, acs2017_vars)
...
mirror_crime_incidence = filter(crime_incidence, crime_incidence$target == 0)
mirror_crime_incidence = na.omit(mirror_crime_incidence, point_x)

# now, to an sf object:
crime_sf <- st_as_sf(na.omit(mirror_crime_incidence), coords = c("point_x", "point_y"), crs = 4326)
crime_sf <- crime_sf %>%
...
dat_crime_MIRROR <- dat_crime
dat_final_MIRROR <- dat_final

remove(mirror_crime_incidence, dat_crime, dat_final)

remove(mapTheme, plotTheme, nn_function)

save.image("~/UPENN/Box Sync/WISE FELLOWSHIP FALL/FINAL_R_CODE/Environments/Environment_Mirror_Dataset.RData")
