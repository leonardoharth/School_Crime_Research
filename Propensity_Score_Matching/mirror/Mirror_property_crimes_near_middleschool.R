...
target_near_school_match <- matchit(near_middleschool ~ 
             log(family_in_pov + .1) +
             (1 - pct_white) +
             pct_hisp +
             pct_young_male +
             ...
             log(distance_retail),
           data = dat_final_MIRROR, method="nearest", ratio=1)

summary(target_near_school_match)

dat_final_MIRROR <- dat_final_MIRROR %>% 
  mutate(
...
Paired_crime_index <- cbind(dat_final_MIRROR[row.names(target_near_school_match$match.matrix),"log_property_crime_index"], dat_final_MIRROR[target_near_school_match$match.matrix,"log_property_crime_index"])
names(Paired_crime_index)[1] <- "Near_middleschool"
names(Paired_crime_index)[2] <- "Not_Near_middleschool"

# Run a T-test
ttest_propertycrime_nearmiddleschool_mirror <- t.test(Paired_crime_index$Near_middleschool, Paired_crime_index$Not_Near_middleschool, paired = TRUE)
