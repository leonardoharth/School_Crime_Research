...
target_near_school_match <- matchit(near_highschool ~ 
             log(family_in_pov + .1) +
             (1 - pct_white) +
             pct_hisp +
             pct_young_male +
...
names(Paired_crime_index)[1] <- "Near_highschool"
names(Paired_crime_index)[2] <- "Not_Near_highschool"

# Run a T-test
ttest_propertycrime_nearhighschool_mirror <- t.test(Paired_crime_index$Near_highschool, Paired_crime_index$Not_Near_highschool, paired = TRUE)
