...
Paired_crime_index <- cbind(dat_final_MIRROR[row.names(target_near_school_match$match.matrix),"log_drug_crime_index"], dat_final_MIRROR[target_near_school_match$match.matrix,"log_drug_crime_index"])
names(Paired_crime_index)[1] <- "Near_middleschool"
names(Paired_crime_index)[2] <- "Not_Near_middleschool"

# Run a T-test
ttest_drugcrime_nearmiddleschool_mirror <- t.test(Paired_crime_index$Near_middleschool, Paired_crime_index$Not_Near_middleschool, paired = TRUE)
