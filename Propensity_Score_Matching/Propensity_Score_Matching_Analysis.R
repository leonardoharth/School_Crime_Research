...
matched_treated <- filter(target_near_school_match_DF, near_school == 1) %>% 
  dplyr::select(log_family_pov, pct_nonwhite, pct_hisp, pct_young_male, log_median_HH_income, avg_litter, log_code_violation,
...
match_df_for_plots <- match_df_for_plots %>% 
  dplyr::select(variable, difference_matched, difference_unmatched) %>% 
  dplyr::rename(var_reg = variable)

match_df_for_plots_long <- melt(match_df_for_plots, id.vars = "var_reg")

match_df_for_plots_long$variable <- if_else(match_df_for_plots_long$variable == "difference_matched",
                                            "Matched", "Unmatched")

ggplot(match_df_for_plots_long) +
  geom_point(aes(x = var_reg, y = value, color = variable), size = 2) +
  coord_flip() +
...
ggplot(Paired_crime_index) +
  ...
  labs(title = "Distribution of crime index differences in pairs", subtitle = "Mean difference in red") + 
  xlab("") + ylab("")

# Run a T-test
ttest_allcrime_nearschool_target <- t.test(Paired_crime_index$Near_School, Paired_crime_index$Not_Near_School, paired = TRUE)
ttest_allcrime_nearschool_target$estimate
