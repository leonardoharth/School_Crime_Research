...
Paired_crime_index <- cbind(dat_final[row.names(target_near_school_match$match.matrix),"log_property_crime_index"], dat_final[target_near_school_match$match.matrix,"log_property_crime_index"])
names(Paired_crime_index)[1] <- "Near_middleSchool"
names(Paired_crime_index)[2] <- "Not_Near_middleSchool"

# Run a T-test
ttest_propertycrime_nearmiddleschool_target <- t.test(Paired_crime_index$Near_middleSchool, Paired_crime_index$Not_Near_middleSchool, paired = TRUE)
