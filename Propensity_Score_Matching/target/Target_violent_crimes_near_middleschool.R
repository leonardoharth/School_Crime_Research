...
Paired_crime_index <- cbind(dat_final[row.names(target_near_school_match$match.matrix),"log_violent_crime_index"], dat_final[target_near_school_match$match.matrix,"log_violent_crime_index"])
names(Paired_crime_index)[1] <- "Near_Middleschool"
names(Paired_crime_index)[2] <- "Not_Near_Middleschool"

# Run a T-test
ttest_violentcrime_nearmiddleschool_target <- t.test(Paired_crime_index$Near_Middleschool, Paired_crime_index$Not_Near_Middleschool, paired = TRUE)
